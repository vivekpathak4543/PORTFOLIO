export const Bio = {
  name: "Vivek Pathak",
  roles: ["Full Stack Developer", "Frontend Developer", "Backend Developer"],
  description:
    "I am a motivated and versatile individual, always eager to take on new challenges. With a passion for learning I am dedicated to delivering high-quality results. With a positive attitude and a growth mindset, I am ready to make a meaningful contribution and achieve great things.",
  github: "https://github.com/vivekpathak4543",
  linkedin: "https://www.linkedin.com/in/vivek-pathak-10624b152/",
  resume: "",
};
export const skills = [
  {
    title: "Frontend",
    id: 1,
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Next Js",
        image:
          "https://seeklogo.com/images/N/next-js-logo-8FCFF51DD2-seeklogo.com.png",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Typescript",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  {
    title: "Backend",
    id: 2,
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
    ],
  },
  {
    title: "Others",
    id: 3,
    skills: [
      {
        name: "Git",
        image: "https://git-scm.com/images/logos/downloads/Git-Icon-1788C.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Netlify",
        image:
          "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Postman",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Figma",
        image:
          "data:image/png;base64,",
      },
    ],
  },
];

export const experiences = [
  {
    id: 1,
    img: "data:image/png;base64,",
    role: "Fullstack Web Development Internship",
    company: "AlmaBetter",
    date: "September 2023 - Present",
    desc: "Experienced intern proficient in front-end and back-end technologies such as React, Express.js, Node.js, Redux, and MongoDB. Skilled in developing robust web applications using the MERN stack, ensuring responsiveness and functionality. Capable of implementing authentication mechanisms and adhering to best practices for high-quality solutions. Additionally, provided subject matter expertise and resolved queries on the forum platform.",
    skills: [
      "ReactJS",
      "Redux",
      "NodeJs",
      "Material UI",
      "HTML",
      "CSS",
      "JavaScript",
      "AWS",
      "MongoDB",
    ],
  },
];

export const projects = [
  {
    id: 1,
    title: "Cryptocurrency Dashboard",
    description:
      "Developed a Cryptocurrency Dashboard, leveraging ReactJS for frontend development and Tailwind CSS for responsive layouts. Integrated third-party Coin Gecko APIs for real-time data retrieval, enabling users to track, analyze, and exchange cryptocurrencies with interactive charts and tables. Seamless integration of third-party APIs ensured efficiency, providing users with up-to-date cryptocurrency data and an intuitive interface for an enhanced user experience.",
    image:
      "https://github.com/vivekpathak4543/project_images/blob/main/crptoScreenshot.png?raw=true",
    tags: [
      "Real-time",
      "data visualization",
      "ReactJS",
      "Redux Toolkit",
      "Coin Gecko API",
      "React Charts",
    ],
    category: "web app",
    github: "https://github.com/vivekpathak4543/Cryptocurrency_Dashboard",
    webapp: "https://cryptocurrency-dash-board.netlify.app/",
  },
  {
    id: 2,
    title: "Get Youtube Subscribers",
    description:
      "Engineered a robust RESTful API system for efficient management of YouTube Subscriber data retrieval, delivering streamlined access to subscriber information in JSON format. Employed Node.js and Express for backend development, with MongoDB as the database solution for effective data storage and management. Designed RESTful API endpoints supporting various functionalities, including retrieval of all subscriber data, access to subscriber names and subscribed channels, and fetching detailed subscriber information by ID, encompassing name, subscribed channel, and subscription date.",
    image:
      "https://github.com/vivekpathak4543/project_images/blob/main/youtubeScreenshot.png?raw=true",
    tags: [
      "RESTful API",
      "JSON",
      "Node.js",
      "Express",
      "MongoDB",
      "Backend development",
    ],
    category: "web app",
    github: "https://github.com/vivekpathak4543/youtube-subscriber",
    webapp: "https://youtube-subscriber-vk.vercel.app/",
  },
  {
    id: 3,
    title: "Entertainment-App",
    description:
      "Discover and enjoy a wide range of movies, TV shows, and music all in one place. With an intuitive interface and personalized recommendations, our app ensures you never miss out on your favorite entertainment. Start streaming now and explore endless possibilities!",
    image:
      "https://github.com/vivekpathak4543/project_images/blob/main/entertainmentScreenshot.png?raw=true",
    tags: [
      "TMDB Api",
      "Json Web Token",
      "Bcrypt",
      "Express",
      "MongoDB",
      "Mongoose",
    ],
    category: "web app",
    github: "https://github.com/vivekpathak4543/Entertainment-App-",
    webapp: "https://entertainment-app-frontend-nqwz.onrender.com/",
  },
];
